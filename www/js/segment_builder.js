...
function getOptions (i) {
  ...
  if (segments[i].type == 'html') {
    ...
    chartData.push(segments[i])

    view = `<div class="form-group">
                <b>${segments[i].title}</b>
                <div class="img">${getChartCanvas()}</div>
  ...
  segments[i].id
    ...
    view += `<div class="${
      segments[i].id
    }" id="explaination" style="display:none">${
      segments[i].answer_text
    }</div><br>`
  } else {
...
function getChartCanvas () {
  return `<canvas id="myChart${chartData.length -
    1}" width="400" height="400"></canvas>`
}
...
function getChart () {
  chartData.forEach((ele, i) => {
    ...
    var custom_labels = ['blue', 'green', 'red', 'yellow']
    var counter = []

    ele.data.forEach(element => {
      custom_labels.forEach(dt => {
        counter.push(element[dt])
      })
    })
