...
var title_input_count = 0;
>>>>>>> af5fa7b97f909e0dc118cc1d83931a94030e2d35
var sign_count = 0;
var sortArray = [];
var MODE;
var pos = 0;
...
=======
	...
	$('#title_input')
		.find('textarea')
		.attr('name', 'title_input_text_' + title_text_count);
	$('#title_input')
		.find('input[type=text]')
		.attr('name', 'title_input_title_' + title_text_count);
	$('#sortable').append($('#title_input').html());
	sortablePositionFunction(isNew, posU);
...
=======
	...
	for (var i = 0; i < title_input_count; i++) {
		...
		position_me = $('input[name="title_input_title_' + i + '"]')
			.parent()
			.parent()
			.data('position');
		...
		$.ajax({
			...
			success: function (data) {
				...
				window.location.replace(
					currentPathName + '?lesson_id=' + data.id
				);
				alert('FlashCard Created!');
			},
		});
...
=======
	...
	if (MODE == 'UPDATE') {
		$.get(
			...
			function (response) {
				...
				$('#lesson_slide').attr(
					'href',
					'/slide.html?lesson_id=' + lesson_id
				);
				$('#lesson_name').val(response.lesson_name);
				var flashcards = response.flashcards;
				flashcards.sort(function (a, b) {
	...
	for (var i = 0; i < video_file_count; i++) {
		var question = $('input[name="video_question_' + i + '"]').val();
		var video = $('input[name="video_' + i + '"]').val();
		position_me = $('input[name="video_question_' + i + '"]')
			.parent()
			.parent()
			.data('position');
	...
	for (var i = 0; i < image_file_count; i++) {
		var question = $('input[name="image_question' + i + '"]').val();
		var video = $('input[name="image_' + i + '"]').val();
		position_me = $('input[name="image_question' + i + '"]')
			.parent()
			.parent()
			.data('position');
